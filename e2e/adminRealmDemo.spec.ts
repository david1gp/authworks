...
test("realm settings demo saves changes and requires a destructive confirmation", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "Disable realm", exact: true }).click()
  await expect(page.getByRole("alert")).toContainText("realm name does not match")

  await page.getByLabel("Enter Renamed realm to confirm", { exact: true }).fill("Renamed realm")
  await page.getByRole("button", { name: "Disable realm", exact: true }).click()
