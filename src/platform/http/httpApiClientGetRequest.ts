...
export async function httpApiClientGetRequest<T>(
  ...
  if (init.cache === "no-store") headers.delete("if-modified-since")
  if (init.cache !== "no-store") {
    if (options.ifModifiedSince instanceof Date)
      headers.set("if-modified-since", httpDateFormat(options.ifModifiedSince))
