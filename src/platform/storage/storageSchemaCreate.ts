...
export function storageSchemaCreate(database: StorageExecutor): Result<void> {
  ...
  try {
    ...
    try {
      database.run(
        "ALTER TABLE realm_login_policies ADD COLUMN session_lifetime_seconds INTEGER CHECK (session_lifetime_seconds IS NULL OR (session_lifetime_seconds >= 1 AND session_lifetime_seconds <= 31536000))",
      )
    } catch (error) {
    ...
    try {
      database.run("ALTER TABLE user_profiles ADD COLUMN picture_url TEXT")
    } catch (error) {
      if (!storageSchemaDuplicateColumnIsExpected(error, "picture_url")) throw error
    }
    try {
      database.run("ALTER TABLE user_profiles ADD COLUMN picture_content_type TEXT")
    } catch (error) {
      if (!storageSchemaDuplicateColumnIsExpected(error, "picture_content_type")) throw error
    }
    ...
    database.run(
      "CREATE TABLE IF NOT EXISTS password_credentials (user_id TEXT PRIMARY KEY NOT NULL, realm_id TEXT NOT NULL, hash TEXT NOT NULL, password_change_required INTEGER NOT NULL DEFAULT 0 CHECK (password_change_required IN (0, 1)), created_at INTEGER NOT NULL CHECK (created_at >= 0), changed_at INTEGER NOT NULL CHECK (changed_at >= 0), version INTEGER NOT NULL CHECK (version > 0), FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE, FOREIGN KEY (realm_id) REFERENCES realms(id) ON DELETE CASCADE)",
    )
    try {
      database.run(
        "ALTER TABLE password_credentials ADD COLUMN password_change_required INTEGER NOT NULL DEFAULT 0 CHECK (password_change_required IN (0, 1))",
      )
    } catch (error) {
      if (!storageSchemaDuplicateColumnIsExpected(error, "password_change_required")) throw error
    }
    database.run(
    ...
    try {
      database.run(
        "ALTER TABLE mfa_challenges ADD COLUMN organization_id TEXT REFERENCES organizations(id) ON DELETE CASCADE",
      )
    } catch (error) {
      if (!storageSchemaDuplicateColumnIsExpected(error, "organization_id")) throw error
    }
