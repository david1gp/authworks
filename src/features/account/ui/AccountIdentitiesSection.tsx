...
export function AccountIdentitiesSection(props: { readonly state: AccountSecurityViewState }) {
  return (
    <div class="grid min-w-0 gap-3 [&>*]:min-w-0">
      ...
      <AuthenticatedSection label={messageTranslate("shell.nav.linkedIdentities")}>
        <Show
          when={props.state.identities().length > 0}
          fallback={<ProductionStatePanel compact state="empty" title={messageTranslate("account.identities.empty")} />}
        >
