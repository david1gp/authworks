...
export function emailOtpCodeCreate(
  ...
  try {
    for (let attempt = 0; attempt < 16; attempt += 1) {
      ...
      if (bytes.length !== 4) return resultErrorCreate(op, "The email OTP code could not be created.")
    ...
    return resultErrorCreate(op, "The email OTP code could not be created.")
  } catch (_error) {
    return resultErrorCreate(op, "The email OTP code could not be created.")
  }
}
