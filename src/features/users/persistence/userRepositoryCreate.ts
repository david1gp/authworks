...
export function userRepositoryCreate(database: StorageExecutor) {
  const userRecordGet = (instanceId: string, userId: string): Result<UserRecord | null> => {
    try {
      const user = database
        .select()
        .from(userTable)
        .where(and(eq(userTable.id, userId), eq(userTable.instanceId, instanceId)))
        .get()
  ...
  return {
    ...
    userGet(instanceId: string, userId: string): Result<UserRecord | null> {
      return userRecordGet(instanceId, userId)
    },

    userList(instanceId: string): Result<UserRecord[]> {
      try {
        const users = database
          .select()
          .from(userTable)
          .where(and(eq(userTable.instanceId, instanceId), ne(userTable.state, "deleted")))
    ...
    userUpdate(instanceId: string, userId: string, input: UserUpdate): Result<UserRecord | null> {
      try {
    ...
    userProfileUpdate(instanceId: string, userId: string, input: UserProfileUpdate): Result<UserRecord | null> {
      try {
        const profile = database
          .update(userProfileTable)
          .set(input)
          .where(and(eq(userProfileTable.userId, userId), eq(userProfileTable.instanceId, instanceId)))
          .returning()
          .get()
