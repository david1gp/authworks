const storageKey = "authworks.oidc.acknowledged-secrets"
...
/**
 * Remembers, for the current browser session only, which one-time secrets have already been
 * acknowledged. Only a non-reversible marker derived from the client identity and the kind
 * is stored: the secret value itself is never persisted, logged, or placed in a URL.
 */
export const oidcAdminSecretAcknowledgementStore = {
  ...
  markerBuild: (clientId: string, kind: string) => `${clientId}:${kind}`,
