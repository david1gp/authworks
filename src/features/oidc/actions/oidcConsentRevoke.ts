...
export function oidcConsentRevoke(options: OidcConsentRevokeOptions): Result<OidcConsentRevokeResponse> {
  ...
  return storageTransactionRun<OidcConsentRevokeResponse>(options.database, (transaction) => {
    ...
    const revoked = repository.consentRevoke(options.instanceId, userId as string, options.clientId, now)
    ...
    const consentVersion = repository.consentEventVersionGet(options.instanceId, userId as string, options.clientId)
