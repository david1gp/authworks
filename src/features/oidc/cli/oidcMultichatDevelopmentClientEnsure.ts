...
export async function oidcMultichatDevelopmentClientEnsure(options: {
  ...
  const op = "oidcMultichatDevelopmentClientEnsure"
  ...
  if (ensured.data.action === "created" && credentialEnvelope === undefined)
    return resultErrorCodedCreate(
      op,
      "The new Multichat development credential envelope was not created.",
      "platform.internal",
    )
  if (ensured.data.action !== "created" && credentialEnvelope !== undefined)
    return resultErrorCodedCreate(
      op,
      "An unexpected Multichat development credential envelope was created.",
      "platform.internal",
    )
  if (credentialEnvelope !== undefined) {
    try {
      ...
      return resultErrorCodedCreate(
        op,
        "The new Multichat development credential could not be handed off.",
        "platform.internal",
      )
    }
  }
