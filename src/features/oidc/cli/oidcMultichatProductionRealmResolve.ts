...
export async function oidcMultichatProductionRealmResolve(api: {
  readonly realmList: (query?: ListQuery) => ReturnType<ReturnType<typeof realmApiClientCreate>["realmList"]>
}): Promise<Result<Realm>> {
  const op = "oidcMultichatProductionRealmResolve"
  const matches: Realm[] = []
  let pageToken: string | undefined
  do {
    const listed = await api.realmList({ pageSize: 100, ...(pageToken === undefined ? {} : { pageToken }) })
    if (!listed.success) return listed
    matches.push(...listed.data.items.filter((realm) => realm.domains.includes(oidcMultichatProductionRealmDomain)))
  ...
  if (realm === undefined || realm.domain !== oidcMultichatProductionRealmDomain)
    return resultErrorCodedCreate(
      op,
