...
export function oidcClientSecretCreate(runtime: OidcClientSecretRuntime = runtimeCreate()): Result<string> {
  ...
  try {
    ...
    if (bytes.length !== 32) return resultErrorCreate(op, "The client secret could not be generated.")
    return resultCreate(oidcBase64UrlEncode(bytes))
  } catch (_error) {
    return resultErrorCreate(op, "The client secret could not be generated.")
  }
}
